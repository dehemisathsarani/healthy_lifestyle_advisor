...
import { ArrowLeft, Brain, Heart, MessageCircle, Calendar, TrendingUp, Play, Target, Smile, User } from 'lucide-react'
import QuickMentalHealthLogin from './QuickMentalHealthLogin'
import { mentalHealthSessionManager } from '../services/MentalHealthSessionManager'
...
export const MentalHealthAgent: React.FC<MentalHealthAgentProps> = ({ onBackToServices, authenticatedUser }) => {
  ...
  const initializeUser = useCallback(async () => {
    try {
      ...
      if (!sessionInitialized) {
        mentalHealthSessionManager.init(
          () => {
            ...
            warningDiv.innerHTML = '⚠️ Mental Health session expiring soon. <button onclick="this.parentElement.remove()" class="ml-2 underline">Dismiss</button>'
            document.body.appendChild(warningDiv)
            setTimeout(() => warningDiv.remove(), 10000)
          },
          () => {
            // Session expired callback
            console.log('Mental Health session expired, but keeping user logged in with saved data')
            const expiredDiv = document.createElement('div')
            expiredDiv.className = 'fixed top-4 right-4 bg-orange-500 text-white px-4 py-2 rounded-lg shadow-lg z-50'
            expiredDiv.innerHTML = '📱 Mental Health session expired - now working offline. <button onclick="this.parentElement.remove()" class="ml-2 underline">OK</button>'
      ...
      const sessionData = await mentalHealthSessionManager.restoreSession()
      if (sessionData?.isValid && sessionData.user) {
        setUser(sessionData.user)
        setIsAuthenticated(true)
        setLastUserEmail(sessionData.user.email)
        console.log('✅ Mental Health session restored for user:', sessionData.user.name)
        ...
        if (authenticatedUser && authenticatedUser.email) {
          ...
          // Try to find existing Mental Health profile for this user
          const savedUsers: UserMentalHealthProfile[] = JSON.parse(
            localStorage.getItem('mentalHealthUserBackups') || '[]'
          )
          
          const existingProfile = savedUsers.find(u => u.email === authenticatedUser.email)
          
          if (existingProfile) {
            // User already has a Mental Health profile, use it
            await mentalHealthSessionManager.createSession(existingProfile)
            ...
            welcomeDiv.innerHTML = `
              <div class="flex items-center space-x-3">
                ...
                <div>
                  <div class="font-bold">Welcome back, ${existingProfile.name}!</div>
                  <div class="text-sm">Mental Health profile loaded successfully</div>
                </div>
              </div>
            `
            ...
            // User needs to create a Mental Health profile
            ...
            authDiv.innerHTML = `
              <div class="flex items-center space-x-3">
                <div class="text-2xl">🔐</div>
                <div>
                  <div class="font-bold">Welcome ${authenticatedUser.name}!</div>
                  <div class="text-sm">Please create your Mental Health profile to continue</div>
                </div>
              </div>
            `
        ...
        const savedEmail = localStorage.getItem('mentalHealthLastEmail')
        if (savedEmail) {
          setLastUserEmail(savedEmail)
          setShowQuickLogin(true)
        } else {
          // No valid session found and no previous email
          console.log('No valid Mental Health session found, user needs to create profile')
        }
      }
    } catch (error) {
      console.error('Error initializing Mental Health user:', error)
    }
  }, [sessionInitialized, authenticatedUser])

  useEffect(() => {
    initializeUser()
  }, [initializeUser])

  // Handle quick login success
  const handleQuickLoginSuccess = useCallback(async (userProfile: SessionUserProfile) => {
    await mentalHealthSessionManager.createSession(userProfile)
    setUser(userProfile)
    setIsAuthenticated(true)
    ...
    welcomeDiv.innerHTML = `
      <div class="flex items-center space-x-3">
        <div class="text-2xl">💙</div>
        <div>
          <div class="font-bold">Welcome back, ${userProfile.name}!</div>
          <div class="text-sm">Mental Health session restored successfully</div>
        </div>
      </div>
    `
    document.body.appendChild(welcomeDiv)
  ...
  const handleLogout = useCallback(async () => {
    ...
    await mentalHealthSessionManager.clearSession()
    setUser(null)
    ...
    logoutDiv.className = 'fixed top-4 right-4 bg-blue-500 text-white px-6 py-3 rounded-lg shadow-lg z-50'
    logoutDiv.innerHTML = `
      <div class="flex items-center space-x-3">
        <div class="text-2xl">👋</div>
        <div>
          <div class="font-bold">Logged out successfully</div>
          <div class="text-sm">Mental Health data saved securely</div>
        </div>
      </div>
    `
    document.body.appendChild(logoutDiv)
    setTimeout(() => logoutDiv.remove(), 4000)
  }, [currentAudio])

  // Handle user profile creation
  const handleUserCreation = useCallback(async (userData: Partial<UserMentalHealthProfile>) => {
    try {
      const newUser: SessionUserProfile = {
        id: Date.now().toString(),
        email: userData.email || authenticatedUser?.email || '',
        name: userData.name || authenticatedUser?.name || '',
        age: userData.age || 25,
        stress_level: userData.stress_level || 'moderate',
        sleep_hours: userData.sleep_hours || 8,
        concerns: userData.concerns || [],
        preferred_activities: userData.preferred_activities || ['meditation', 'music', 'breathing'],
        mood_goals: userData.mood_goals || ['reduce stress', 'improve sleep', 'increase happiness']
      }

      // Save to session manager
      await mentalHealthSessionManager.createSession(newUser)
      
      // Save to user backups
      const savedUsers: SessionUserProfile[] = JSON.parse(
        localStorage.getItem('mentalHealthUserBackups') || '[]'
      )
      
      const existingIndex = savedUsers.findIndex(u => u.email === newUser.email)
      if (existingIndex >= 0) {
      ...
      localStorage.setItem('mentalHealthUserBackups', JSON.stringify(savedUsers))
      localStorage.setItem('mentalHealthLastEmail', newUser.email)
      ...
      welcomeDiv.innerHTML = `
        <div class="flex items-center space-x-3">
          <div class="text-2xl">🎉</div>
          <div>
            <div class="font-bold">Welcome ${newUser.name}!</div>
            <div class="text-sm">Mental Health profile created successfully</div>
          </div>
        </div>
      `
      document.body.appendChild(welcomeDiv)
  ...
  // Global stop music function for HTML button access
  useEffect(() => {
    // Create global stop music function for HTML buttons
    (window as any).globalStopMusic = () => {
      ...
      // Remove YouTube player modal
      const youtubeModal = document.getElementById('youtube-player-modal')
      if (youtubeModal) {
        youtubeModal.remove()
      }
      ...
      const youtubePlayer = document.getElementById('youtube-music-player')
      if (youtubePlayer) {
      ...
      existingMessages.forEach(msg => {
        if (msg.textContent?.includes('🎵') || msg.textContent?.includes('Loading') || msg.textContent?.includes('Playing') || msg.textContent?.includes('Preparing') || msg.textContent?.includes('YouTube')) {
          msg.remove()
        }
      })
    ...
    // Cleanup function
    return () => {
      delete (window as any).globalStopMusic
    }
  }, [currentAudio])
  ...
  const logMoodEntry = async (entry: Omit<MoodEntry, 'id' | 'date'>) => {
    ...
    if (newEntry.mood_rating <= 2) {
      console.log('😔 Low mood detected, getting interventions...')
      ...
      try {
        ...
        console.log('🎭 Interventions received:', interventions)
        ...
        console.log('🔧 Showing intervention modal...')
  ...
  const getLowMoodInterventions = async (currentFeeling?: string, musicPreference?: string) => {
    console.log('🔄 Getting low mood interventions for:', { currentFeeling, musicPreference })
    ...
    // Music recommendations based on mood and preference - Enhanced with verified YouTube IDs
    const musicSuggestions = {
      uplifting: [
        { title: "Happy", artist: "Pharrell Williams", mood_type: "uplifting", duration: "3:53", youtube_id: "ZbZSe6N_BXs", spotify_id: "60nZcImufyMA1MKQY3dcCH" },
        { title: "Can't Stop the Feeling", artist: "Justin Timberlake", mood_type: "uplifting", duration: "3:56", youtube_id: "ru0K8uYEZWw", spotify_id: "1WkMMavIMc4JZ8cfMmxHkI" },
        { title: "Walking on Sunshine", artist: "Katrina and the Waves", mood_type: "uplifting", duration: "3:59", youtube_id: "iPUmE-tne5U", spotify_id: "05wIrZSwuaVWhcv5FfqeH0" },
        { title: "Good as Hell", artist: "Lizzo", mood_type: "uplifting", duration: "2:39", youtube_id: "SmbmeOgWsqE", spotify_id: "1PckUlxKqWQs2dEXBEH297" },
        { title: "Uptown Funk", artist: "Mark Ronson ft. Bruno Mars", mood_type: "uplifting", duration: "4:30", youtube_id: "OPf0YbXqDm0", spotify_id: "32OlwWuMpZ6b0aN2RZOeMS" },
        { title: "I Will Survive", artist: "Gloria Gaynor", mood_type: "uplifting", duration: "3:18", youtube_id: "FHhZPp08s74", spotify_id: "7LZXuPBjvcwW3sQZPEp7mY" },
        { title: "Don't Stop Me Now", artist: "Queen", mood_type: "uplifting", duration: "3:29", youtube_id: "HgzGwKwLmgM", spotify_id: "5T8EDUDqKcs6OSOwEsfqG7" },
        { title: "Three Little Birds", artist: "Bob Marley", mood_type: "uplifting", duration: "3:00", youtube_id: "zaGUr6wzyT8", spotify_id: "6JV2JOEocMgcZxYSZelKcc" }
      ],
      calm: [
        { title: "Weightless", artist: "Marconi Union", mood_type: "calm", duration: "8:08", youtube_id: "1ZYbU82GVz4", spotify_id: "2WZqITJT3HLl0NKpK7E9nQ" },
        { title: "Clair de Lune", artist: "Claude Debussy", mood_type: "calm", duration: "5:20", youtube_id: "CvFH_6DNRCY", spotify_id: "4Iojp1NrJG0t5YJ0m6Rl1w" },
        { title: "Aqueous Transmission", artist: "Incubus", mood_type: "calm", duration: "7:49", youtube_id: "eQK7KSTQfaw", spotify_id: "3b1P7eaP5XQKlWMSr8Z4I3" },
        { title: "River", artist: "Joni Mitchell", mood_type: "calm", duration: "4:00", youtube_id: "3NH-ctddY9o", spotify_id: "7FTm2MLvlVRUr1AJsIz6ey" },
        { title: "Mad World", artist: "Gary Jules", mood_type: "calm", duration: "3:07", youtube_id: "4N3N1MlvVc4", spotify_id: "3JOVTQ5xWe2bLUUx8BKML9" },
        { title: "The Sound of Silence", artist: "Simon & Garfunkel", mood_type: "calm", duration: "3:05", youtube_id: "4fWyzwo1xg0", spotify_id: "5u1n1kITHCxxp8twBcZxWy" },
        { title: "Gymnopédie No.1", artist: "Erik Satie", mood_type: "calm", duration: "3:32", youtube_id: "S-Xm7s9eGxU", spotify_id: "07B7qJ6o4VY8xXkj7JJ7I1" },
        { title: "Spa Relaxation Music", artist: "Meditation Relax Music", mood_type: "calm", duration: "60:00", youtube_id: "lFcSrYw-ARY", spotify_id: "N/A" }
      ],
      energetic: [
        ...
        { title: "Pump It", artist: "Black Eyed Peas", mood_type: "energetic", duration: "3:33", youtube_id: "ZaI2IlHwmgQ", spotify_id: "1376z5kx6FxbnPeR6N2hGF" },
        { title: "Eye of the Tiger", artist: "Survivor", mood_type: "energetic", duration: "4:05", youtube_id: "btPJPFnesV4", spotify_id: "2KH16WveTQWT6KOG9Rg6e2" }
      ],
      soothing: [
        ...
        { title: "Hurt", artist: "Johnny Cash", mood_type: "soothing", duration: "3:38", youtube_id: "vt1Pwfnh5pc", spotify_id: "6p0p7z2O9bOKzsZQ4e2SXV" },
        { title: "Hallelujah", artist: "Jeff Buckley", mood_type: "soothing", duration: "6:53", youtube_id: "y8AWFf7EAc4", spotify_id: "2gyxeu8Yq6UiHt4tN1a2Y9" }
      ],
      anxiety_relief: [
        ...
        { title: "Relaxing Piano Music", artist: "Peaceful Piano", mood_type: "anxiety_relief", duration: "30:00", youtube_id: "kxopViU98Xo", spotify_id: "N/A" },
        { title: "Nature Sounds Rain", artist: "Nature Sounds", mood_type: "anxiety_relief", duration: "60:00", youtube_id: "q76bMs-NwRk", spotify_id: "N/A" }
      ],
      anger_management: [
  ...
  const showEnhancedLowMoodIntervention = (interventions: any, moodEntryId: string) => {
    ...
    window.moodModal = {
      ...
      playMusic: (title: string, artist: string) => {
        console.log('🎵 playMusic called with:', { title, artist })
        ...
        // Remove any existing YouTube players
        ...
        try {
          // Find the track with YouTube ID
          let youtubeId = null
          let trackInfo = null
          
          // Search through interventions.music_tracks for the matching track
          const musicTracks = interventions.music_tracks || []
          const foundTrack = musicTracks.find((track: any) => 
            track.title.toLowerCase() === title.toLowerCase() && 
            track.artist.toLowerCase() === artist.toLowerCase()
          )
          
          if (foundTrack && foundTrack.youtube_id) {
            youtubeId = foundTrack.youtube_id
            trackInfo = foundTrack
          } else {
            // Enhanced fallback: Try to find any track with similar artist or title
            const fallbackTrack = musicTracks.find((track: any) => 
              track.artist.toLowerCase().includes(artist.toLowerCase().split(' ')[0]) ||
              track.title.toLowerCase().includes(title.toLowerCase().split(' ')[0])
            )
            
            if (fallbackTrack && fallbackTrack.youtube_id) {
              youtubeId = fallbackTrack.youtube_id
              trackInfo = fallbackTrack
              console.log('Using fallback track:', fallbackTrack.title, 'by', fallbackTrack.artist)
            }
          }

          if (!youtubeId) {
            // Enhanced fallback: Use a proven working YouTube music video
            console.warn('No YouTube ID found for track:', title, artist, 'using fallback music')
            
            // Use a universally accessible relaxing music video as fallback
            youtubeId = "1ZYbU82GVz4" // "Weightless" by Marconi Union - scientifically proven to reduce anxiety
            trackInfo = {
              title: "Relaxing Background Music (Weightless)",
              artist: "Marconi Union",
              mood_type: "therapeutic",
              duration: "8:08",
              youtube_id: youtubeId
            }
            
            const fallbackMessage = document.createElement('div')
            fallbackMessage.className = 'fixed top-4 left-4 bg-blue-500 text-white px-6 py-3 rounded-lg shadow-lg z-50'
            fallbackMessage.innerHTML = `🎵 Playing therapeutic music instead of "${title}" - scientifically proven to help reduce stress`
            document.body.appendChild(fallbackMessage)
            setTimeout(() => fallbackMessage.remove(), 5000)
          }

          // Create enhanced YouTube player modal with better controls
          const playerModal = document.createElement('div')
          playerModal.className = 'fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-60'
          playerModal.id = 'youtube-player-modal'
          
          // Enhanced player with better YouTube embed parameters for autoplay and reliability
          playerModal.innerHTML = `
            <div class="bg-white rounded-2xl p-6 max-w-4xl w-full mx-4 max-h-[95vh] overflow-y-auto shadow-2xl">
              <div class="flex justify-between items-center mb-4">
                <div class="flex-1">
                  ...
                  <p class="text-gray-600 font-medium">${trackInfo?.title || title} by ${trackInfo?.artist || artist}</p>
                  <p class="text-sm text-blue-600">Duration: ${trackInfo?.duration || 'Unknown'} • ${trackInfo?.mood_type || 'Therapeutic'} music</p>
                </div>
                <div class="flex gap-2">
                  <button onclick="this.closest('#youtube-player-modal').style.display='none';" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors text-sm font-medium shadow-md">
                    📱 Minimize
                  </button>
                  <button onclick="window.globalStopMusic()" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg transition-colors text-sm font-medium shadow-md">
                    ⏹️ Stop & Close
                  </button>
                </div>
              </div>
              ...
              <div class="relative rounded-lg overflow-hidden shadow-lg bg-black">
                <iframe 
                  ...
                  width="100%" 
                  height="450" 
                  src="https://www.youtube.com/embed/${youtubeId}?autoplay=1&controls=1&rel=0&modestbranding=1&showinfo=0&fs=1&cc_load_policy=0&iv_load_policy=3&autohide=1&start=0"
                  frameborder="0" 
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share; fullscreen" 
                  allowfullscreen
                  loading="eager"
                  title="YouTube Music Player: ${trackInfo?.title || title}">
                </iframe>
                ...
                <div class="flex items-center gap-3">
                  ...
                  <div class="flex-1">
                    <p class="text-sm text-gray-700 font-medium">
                      This ${trackInfo?.mood_type || 'therapeutic'} track is specifically selected to help with your current mood.
                    </p>
                    <p class="text-xs text-gray-500 mt-1">
                      You can minimize this player to continue using the app while listening to music. The music will continue playing in the background.
                    </p>
                  </div>
              ...
              <div class="mt-4 flex justify-center gap-3">
                <button onclick="this.closest('#youtube-player-modal').style.display='none';" class="px-6 py-2 bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-200 hover:to-gray-300 text-gray-700 rounded-lg transition-all duration-200 text-sm font-medium shadow-md">
                  🎧 Continue with music in background
                </button>
                <button onclick="navigator.share && navigator.share({title: 'Great Music for Mental Health', url: 'https://youtube.com/watch?v=${youtubeId}'})" class="px-4 py-2 bg-gradient-to-r from-blue-100 to-blue-200 hover:from-blue-200 hover:to-blue-300 text-blue-700 rounded-lg transition-all duration-200 text-sm font-medium shadow-md">
                  📤 Share
                </button>
              </div>
            </div>
          ...
          const handleKeyPress = (e: KeyboardEvent) => {
            if (e.ctrlKey || e.metaKey) {
              switch (e.key) {
                ...
                case 'q':
                  e.preventDefault()
                  window.globalStopMusic?.()
                  break
              }
            }
            // Space bar to toggle player visibility (only if modal exists)
            if (e.code === 'Space' && !e.ctrlKey && !e.metaKey && !e.altKey) {
              const modal = document.getElementById('youtube-player-modal')
              if (modal && document.activeElement?.tagName !== 'INPUT' && document.activeElement?.tagName !== 'TEXTAREA') {
                e.preventDefault()
                modal.style.display = modal.style.display === 'none' ? 'flex' : 'none'
              }
            }
          ...
          // Cleanup keyboard listeners when music stops
          const originalStopMusic = window.globalStopMusic
          window.globalStopMusic = () => {
          ...
          // Show enhanced success notification with better interactivity
          const musicMessage = document.createElement('div')
          musicMessage.className = 'fixed top-4 left-4 bg-gradient-to-r from-green-500 to-green-600 text-white px-6 py-3 rounded-lg shadow-lg z-50 cursor-pointer hover:shadow-xl transition-all duration-200'
          musicMessage.innerHTML = `
            <div class="flex items-center gap-3">
              <div class="animate-pulse text-2xl">🎵</div>
              <div>
                <div class="font-bold text-lg">YouTube Music Player Active</div>
                <div class="text-sm">${trackInfo?.title || title} by ${trackInfo?.artist || artist}</div>
                <div class="text-xs opacity-90">Click anywhere to reopen player window</div>
              </div>
              <button onclick="event.stopPropagation(); document.getElementById('youtube-player-modal').style.display='flex';" class="ml-2 px-3 py-1 bg-green-700 hover:bg-green-800 rounded text-xs font-medium transition-colors">
                🎬 Open Player
              </button>
            </div>
          `
          ...
          // Keep the notification visible while music is playing - click to reopen
          musicMessage.onclick = () => {
            const modal = document.getElementById('youtube-player-modal')
            if (modal) {
          ...
          // Add an enhanced floating mini-control with better design
          const miniPlayer = document.createElement('div')
          miniPlayer.className = 'fixed bottom-4 right-4 bg-gradient-to-r from-white to-gray-50 border-2 border-green-500 rounded-xl shadow-2xl p-4 z-50 hover:shadow-3xl transition-all duration-300 hover:scale-105'
          miniPlayer.id = 'mini-music-player'
          miniPlayer.innerHTML = `
            <div class="flex items-center gap-3">
              <div class="relative">
                <div class="animate-pulse text-green-500 text-2xl">🎵</div>
                <div class="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full animate-pulse"></div>
              </div>
              <div class="text-sm min-w-0 flex-1">
                <div class="font-bold text-gray-800 truncate max-w-32" title="${trackInfo?.title || title}">${trackInfo?.title || title}</div>
                <div class="text-gray-600 text-xs truncate max-w-32" title="${trackInfo?.artist || artist}">${trackInfo?.artist || artist}</div>
                <div class="text-green-600 text-xs font-medium">${trackInfo?.mood_type || 'Therapeutic'} music</div>
              </div>
              <div class="flex flex-col gap-1">
                <button onclick="document.getElementById('youtube-player-modal').style.display='flex';" class="px-2 py-1 bg-blue-500 text-white rounded text-xs hover:bg-blue-600 transition-colors font-medium shadow-sm">
                  🎬 Open
                </button>
                <button onclick="window.globalStopMusic()" class="px-2 py-1 bg-red-500 text-white rounded text-xs hover:bg-red-600 transition-colors font-medium shadow-sm">
                  ⏹️ Stop
                </button>
              </div>
            </div>
            <div class="mt-2 text-center">
              <div class="text-xs text-gray-500">Music playing in background</div>
          ...
          // Auto-hide main notification after 5 seconds, but keep mini-player
          setTimeout(() => {
            if (musicMessage.parentElement) musicMessage.remove()
          }, 5000)
          
          // Store reference to the modal for cleanup
          setCurrentAudio(playerModal as any)
          ...
          errorMessage.className = 'fixed top-4 left-4 bg-red-500 text-white px-6 py-3 rounded-lg shadow-lg z-50'
          errorMessage.innerHTML = `❌ Error playing "${title}": ${err.message}`
          document.body.appendChild(errorMessage)
          setTimeout(() => errorMessage.remove(), 4000)
        }
        
        // Always save interaction to history when music is requested
        console.log('💾 Saving music interaction to history')
      ...
      stopMusic: () => {
        if (currentAudio) {
          // Handle different types of audio players
          if (typeof currentAudio.pause === 'function') {
            // HTML Audio element
            currentAudio.pause()
            currentAudio.currentTime = 0
          ...
          } else if ((currentAudio as any).id === 'youtube-player-modal') {
        ...
        // Remove YouTube player modal if it exists
        const youtubeModal = document.getElementById('youtube-player-modal')
        if (youtubeModal) {
          youtubeModal.remove()
        }
        ...
        // Remove any standalone YouTube player
        const youtubePlayer = document.getElementById('youtube-music-player')
        if (youtubePlayer) {
          youtubePlayer.remove()
        }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
        <div className="max-w-4xl mx-auto px-4 py-8 space-y-8">
          ...
          {/* Show Quick Login if available */}
          {showQuickLogin && lastUserEmail && (
            <QuickMentalHealthLogin
              lastUserEmail={lastUserEmail}
              onLogin={handleQuickLoginSuccess}
              onCancel={() => setShowQuickLogin(false)}
              onCreateNew={() => setShowQuickLogin(false)}
            />
          )}
          ...
          {!showQuickLogin && (
            <>
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-white/50 p-8 text-center shadow-xl">
                ...
                {/* Check if user is already authenticated in main app */}
                {authenticatedUser && (
                  <div className="bg-blue-50 rounded-lg p-4 mb-6">
                    <p className="text-blue-800 font-medium">
                      👋 Hi {authenticatedUser.name}! We see you're already logged in. 
                      Create your Mental Health profile to get started.
                    </p>
                  </div>
                )}
              </div>
              ...
              <ProfileForm onSubmit={handleUserCreation} />
            </>
          )}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              ...
              className="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 hover:text-red-600 transition-all duration-200 hover:bg-white/50 rounded-lg border border-gray-200 hover:border-red-200"
              title="Logout from Mental Health Agent"
            >
        ...
        {/* Music Player Status - Show when music is playing */}
        {isPlaying && currentTrack && (
          <div className="bg-gradient-to-r from-green-100 to-blue-100 border border-green-300 rounded-xl p-4 shadow-lg">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className="animate-pulse text-green-600 text-2xl">🎵</div>
                <div>
                  <p className="font-semibold text-gray-800">Now Playing: {currentTrack.title}</p>
                  <p className="text-sm text-gray-600">by {currentTrack.artist}</p>
                </div>
              </div>
              <button
                onClick={() => window.globalStopMusic?.()}
        ...
        {audioError && (
          <div className="bg-red-100 border border-red-300 rounded-xl p-4 shadow-lg">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="font-semibold text-red-800">Audio Error</p>
        ...
        {audioLoading && (
          <div className="bg-blue-100 border border-blue-300 rounded-xl p-4 shadow-lg">
            <div className="flex items-center space-x-3">
              <div className="animate-spin text-blue-600 text-xl">🔄</div>
              <div>
                <p className="font-semibold text-blue-800">Loading Music...</p>
                <p className="text-sm text-blue-600">Please wait while we prepare your audio experience</p>
              </div>
            </div>
          </div>
        )}
